...
const AdminUsers = () => {
  ...
  return (
    <div style={containerStyle}>
      <h2 style={{ color: 'var(--gold)', marginBottom: '20px' }}>User Directory</h2>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u._id} style={rowStyle}>
                ...
                <td style={tdStyle}>
                  <span style={{ background: u.role === 'admin' ? 'rgba(234,88,12,0.2)' : 'rgba(16,185,129,0.2)', color: u.role === 'admin' ? 'var(--gold)' : 'var(--success)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 'bold' }}>
                    {u.role.toUpperCase()}
                  </span>
                </td>
